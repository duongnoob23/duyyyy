...
function DataLogs() {
  ...
  const [api, setApi] = useState(
    URL +
      `/function?searchKey=${params.searchKey}&searchValue=${params.searchValue}&page=${params.page}&limit=${params.limit}`
  );
  ...
  useEffect(() => {
    ...
    setApi(
      URL +
        `/function?searchKey=${params.searchKey}&searchValue=${params.searchValue}&page=${params.page}&limit=${params.limit}&sortKey=${params.sortKey}&sortValue=${params.sortValue}`
    );
  }, [params]);
      ...
      <div className="action">
        ...
        <div className="option">
          ...
          <div className="select">
            ...
            >
              <option className="select__option1" value={"temperature"}>
                Temperature
              </option>
              <option className="select__option2" value={"humidity"}>
                Humidity
              </option>
              <option className="select__option3" value={"light"}>
                Light
              </option>
              <option className="select__option4" value={"time"}>
                Time
              </option>

              <option className="select__option6" value="" hidden>
                --Hãy chọn giá trị--
              </option>
            </select>
          </div>
          <div className="select2">
            ...
            >
              <option className="select2__option1" value={10}>
                10 hàng
              </option>
              <option className="select2__option2" value={20}>
                20 hàng
              </option>
              <option className="select2__option3" value={30}>
                30 hàng
              </option>
              <option className="select2__option4" value="" hidden>
                --Hãy chọn giá trị--
              </option>
            </select>
          </div>
          ...
          <table className="table">
            <thead className="table__head">
              <tr className="table-row__head">
                <th
                  classname="table__id table-row__item"
                  onClick={() => handleClickSort("id")}
                >
                  ID
                </th>
                <th
                  className="table__temperature table-row__item"
                  onClick={() => handleClickSort("temperature")}
                >
                  Nhiệt độ (°C)
                </th>
                <th
                  className="table__humidity table-row__item"
                  onClick={() => handleClickSort("humidity")}
                >
                  Độ ẩm (%)
                </th>
                <th
                  className="table__light table-row__item"
                  onClick={() => handleClickSort("light")}
                >
                  Ánh sáng (lux)
                </th>
                {/* <th className="table__time table-row__item" onClick={() => handleClickSort("windspeed")}>Tốc độ gió</th> */}
                <th
                  className="table__time table-row__item"
